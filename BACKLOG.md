...
- Provision against a real tenancy: `setup` → `tf apply` (creates infra, self-provisions the jump host via cloud-init, and generates `./wallet`) → `build` → `run` → `health`.
...
- **The jump host self-provisions via cloud-init**, not an SSH push. `tf apply` zips the Ansible role to Object Storage, issues a PAR URL, and the host downloads + runs it locally (`ansible-playbook -c local`). This needs the jump host to have egress to Object Storage and the dnf mirrors (the managed default security list provides it) and the PAR to be unexpired. Re-running the experiment (`socks_auth_method=username`) is done by SSHing in and re-running `/root/ansible/socks5.yml` locally — there is no `manage.py provision`.
- **The wallet is generated by Terraform into `./wallet` on the client and never goes to the jump host.** The jump host is a dumb relay; only the client (running the app) holds the wallet. `auth_mode = tls` skips wallet generation entirely.
